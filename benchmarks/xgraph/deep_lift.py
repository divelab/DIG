...
def pipeline(config):
    ...
    deep_lift_perturb = DeepLIFT(model, explain_graph=config.models.param.graph_classification)
    ...
    if config.models.param.graph_classification:
        ...
        for node_idx in node_indices:
            ...
            if isinstance(dataset, SynGraphDataset):
                ...
                motif_edge_mask = motif_edge_mask[deep_lift.hard_edge_mask].float()
                # jump the all one mask
                if not (motif_edge_mask == 1).all():
                    edge_masks = [edge_mask[deep_lift.hard_edge_mask] for edge_mask in edge_masks]
                    ...
                    for label_idx in range(dataset.num_classes):
                        all_roc_aucs.append([])
                    perturb_input_list = perturb_input(data, deep_lift.hard_edge_mask, deep_lift.subset)
                    for perturb_data in perturb_input_list:
                        new_prediction = model(perturb_data)[deep_lift.new_node_idx].argmax(dim=-1)
                        perturb_walks, perturb_masks, perturb_related_preds = \
                            deep_lift_perturb(perturb_data.x,
                                              perturb_data.edge_index,
                                              node_idx=deep_lift.new_node_idx,
                                              sparsity=config.explainers.sparsity,
                                              num_classes=dataset.num_classes)
    ...
    recorder.append(experiment_settings=['deep_lift', f"{config.explainers.sparsity}"],
...
if __name__ == '__main__':
    import sys
    sys.argv.append('explainers=deep_lift')

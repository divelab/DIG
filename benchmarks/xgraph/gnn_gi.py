...
def pipeline(config):
    ...
    gnngi_explainer_perturb = GNN_GI(model, explain_graph=config.models.param.graph_classification)
    ...
    if config.models.param.graph_classification:
        ...
        for node_idx in node_indices:
            ...
            if isinstance(dataset, SynGraphDataset):
                ...
                motif_edge_mask = motif_edge_mask[gnngi_explainer.hard_edge_mask].float()
                if not (motif_edge_mask == 1).all():
                    edge_masks = [edge_mask[gnngi_explainer.hard_edge_mask] for edge_mask in edge_masks]
                    ...
                    for label_idx in range(dataset.num_classes):
                        all_roc_aucs.append([])
                    perturb_input_list = perturb_input(data, gnngi_explainer.hard_edge_mask, gnngi_explainer.subset)
                    for perturb_data in perturb_input_list:
                        new_prediction = model(perturb_data)[gnngi_explainer.new_node_idx].argmax(dim=-1)
                        perturb_walks, perturb_edge_masks, perturb_related_preds = \
                            gnngi_explainer_perturb(perturb_data.x,
                                                    perturb_data.edge_index,
                                                    node_idx=gnngi_explainer.new_node_idx,
                                                    sparsity=config.explainers.sparsity,
                                                    num_classes=dataset.num_classes)

...
parser.add_argument('--save_result_file', type=str, default='./result/cons_trainset.txt', help='Save evaluation result')
...
parser.add_argument('--exp_name', type=str, default='goal_plogp_trainset', help='If save generated melucules')
...
if __name__ == '__main__':
    ...
    # smallest 800 plogp molecules in test set
    inds2 = sorted(range(len(train_prop)), key=lambda k: train_prop[k], reverse=False)
    ...
    init_dataset = SimpleDataset(train_smiles[:800], train_scores[:800], transform_fn)
...
#         os.makedirs(args.save_dir)
    ...
    for run in tqdm(range(args.runs)):
        ...
        optim_dict, save_dict = generate(model, init_dataloader, n_atom, n_atom_type, n_edge_type, device, atomic_num_list)
        update_total_optim_dict(total_optim_dict, optim_dict)

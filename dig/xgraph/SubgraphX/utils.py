...
def plot_sentence(graph, coalition=None, title_sentence=None, figname=None, show=False):
    pos = nx.kamada_kawai_layout(graph)

    words = nx.get_node_attributes(graph, "label")

    if coalition is not None:
        pos_coalition = {k: v for k, v in pos.items() if k in coalition}
        ...
        edgelist_coalition = [(n_frm, n_to) for (n_frm, n_to) in graph.edges() if n_frm in coalition and n_to in coalition]
        nx.draw_networkx_edges(graph, pos=pos_coalition, edgelist=edgelist_coalition, width=5, edge_color='yellow')
    ...
    nx.draw_networkx_labels(graph, pos, words)

    plt.axis('off')
    plt.title('\n'.join(wrap(title_sentence, width=50)))
...
def plot_subgraph(graph, coalition, node_type_dict=None, node_color=None,
                  title_sentence=None, figname=None):
    if node_color is None:
        node_color = ['orange', 'lime', 'blue', 'maroon', 'brown', 'darkslategray', 'paleturquoise',
                      'darksalmon', 'slategray', 'mediumseagreen', 'mediumblue', 'orchid']
    pos = nx.kamada_kawai_layout(graph)
    ...
    for n in attr:
        ...
        #     labels[n] = Chem.PeriodicTable.GetElementSymbol(Chem.GetPeriodicTable(), int(attr[n]))
        # else:
        #     labels[n] = node_type_dict[attr[n]]
        # colors.append(node_color[attr[n] % len(node_color)]) '#FFD965'
    ...
    edgelist_coalition = [(n_frm, n_to) for (n_frm, n_to) in graph.edges() if n_frm in coalition and n_to in coalition]
    nx.draw_networkx_edges(graph, pos=pos_coalition, edgelist=edgelist_coalition, width=5, edge_color='#D9958F')

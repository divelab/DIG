...
from Configures import logger, data_args, train_args, model_args


def train_MUTAG_before():
    ...
    logger.info(f'The total num of dataset is {data_size}')
    ckpt_path = f"./checkpoint/{data_args.dataset_name}/{model_args.model_name}_best.pth"
    ...
    for epoch in range(train_args.max_epochs):
        ...
        for i in range(data_size):
            optimizer.zero_grad()
            data = dataset[int(orders[i])]
            data = Batch.from_data_list([data])
            logits, probs, _ = gnnNets(data)
        ...
        logger.info(f"Epoch:{epoch}  |Loss: {np.average(loss_list):.3f} | Acc: {np.average(acc):.3f}")
        ...
        if (np.average(acc) >= best_acc):
            ...
            if not os.path.isdir(os.path.join('checkpoint', data_args.dataset_name)):
                os.mkdir(os.path.join('checkpoint', data_args.dataset_name))
            logger.info("Saving...")
            torch.save(state, ckpt_path)
            best_acc = np.average(acc)

    state_dict = torch.load(ckpt_path)['net']
    gnnNets.update_state_dict(state_dict)
    gnnNets.eval()
    acc = []
    for i in range(data_size):
        data = dataset[i]
        ...
        _, prediction = torch.max(logits, -1)
        acc.append(prediction.eq(data.y).item())
    logger.info(f"Test | Acc: {np.average(acc):.3f}")
    logger.info(f'best acc is {best_acc}')  # best acc is 0.9627659574468085
...
def train_MUTAG():
    ...
    #     logger.info(f"Train Epoch:{epoch}  |Loss: {epoch_loss:.3f} | "
    #                 f"Acc: {epoch_acc:.3f}")
    #
    #     # only save the best model
    ...
    # logger.info(f"The best validation accuracy is {best_acc}.")
    # report test msg
    ...
    logger.info(f"Test: | Loss: {test_state['loss']:.3f} | Acc: {test_state['acc']:.3f}")
...
def train_GC():
    ...
    logger.info(f"graphs {len(dataset)}, avg_nodes{avg_nodes}, avg_edge_index_{avg_edge_index/2}")
    best_acc = 0.0
    best_loss = 1000.0
    data_size = len(dataset)
    logger.info(f'The total num of dataset is {data_size}')
    ...
    #
    #     # report train msg
    #     logger.info(f"Train Epoch:{epoch}  |Loss: {np.average(loss_list):.3f} | "
...
def train_NC():
    ...
    #         logger.info(f'Epoch {epoch}, Train Loss: {eval_info["train_loss"]:.4f}, '
    ...
    eval_info = evaluate_NC(data, gnnNets_NC, criterion)
    logger.info(f'Test Loss: {eval_info["test_loss"]:.4f}, Test Accuracy: {eval_info["test_acc"]:.3f}')
